...
| 4 | Access Control / RBAC Model | Eight baseline roles with mandatory separations of duties (System Administrator excluded from content access, Reviewer/Approver separated by default, Auditor independent). | ADR-004-AccessControlRBACModel.md |
...
| 5 | Data Classification Framework (C1–C4) | Four classification levels (Organizational Confidential, Personal Information, Sensitive Personal Information, Audit/Governance Metadata) driving retention, access, and export handling. | PrivacyRequirements.md §6 |
| 6 | Seven-State Transcript Lifecycle | Recording Received → Processing → Draft Transcript → Review Required → Reviewed → Approved → Archived, with defined backward transitions. | FunctionalRequirements.md §3.1 |
