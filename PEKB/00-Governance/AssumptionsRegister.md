...
| AR-003 | No desktop technology stack has been chosen; it is assumed any stack considered must operate without administrator rights and without Docker. | Foundation Review v0.1, Required Decision #3 | Blocks `DesktopArchitecture.md` and `ReleaseStrategy.md`; risk of costly rework if assumed incorrectly. | High | **Open — premise now binding per ADR-005** (no admin rights / no Docker/container dependency is ratified as a constraint; the stack *choice* itself remains Open and is an Engineering-phase decision bounded by ADR-005 §4). | Principal Software Architect + DevOps/Deployment Engineer |
